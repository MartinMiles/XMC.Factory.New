...
# 1) Load JSON maps
$jsonPath = [System.Web.Hosting.HostingEnvironment]::MapPath("~/App_Data/home.json")
if (-not (Test-Path $jsonPath)) { Throw "more.json not found: $jsonPath" }
$jsonRoot = [Newtonsoft.Json.Linq.JToken]::Parse((Get-Content $jsonPath -Raw))
...
Write-Host "All done for $itemPath"
